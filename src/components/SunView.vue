...
const { initSun, curSolarTerms, onChangeTerm, sunlightPosition, progress, run, stop, timeInfo, isRunning } = useSun();
...
const initClick = () => {
    ...
    document.addEventListener('mouseup', event => {
        if (!isDragging) {
            ...
            if (intersects.length > 0 && /^\d{1,2}[A-Z]?./.test(intersects?.[0]?.object?.name)) {
                // 拿到最近的物体
...
const backSandbox = () => {
    ...
    tweenCamera(threeObj.camera, threeObj.camera.position, originalCameraPosition.value, 666, 0, () => {
